  ...
  <div class="space-y-8">
    ...
    <div class="space-y-5">
      <div class="flex items-center gap-2 mb-2">
         ...
         <h3 class="text-sm font-medium text-white/80 tracking-wide">群组设置 / Group Settings</h3>
      </div>
      
      <div class="group">
        <label class="block text-[10px] font-medium text-white/40 uppercase tracking-widest mb-2 group-focus-within:text-[#7A9D8C] transition-colors">Group Chat Name</label>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="group">
          <label class="block text-[10px] font-medium text-white/40 uppercase tracking-widest mb-2 group-focus-within:text-[#7A9D8C] transition-colors">OS Style</label>
          <div class="relative">
            <select v-model="chatStore.deviceType" class="w-full appearance-none bg-white/5 hover:bg-white/10 border border-transparent focus:border-[#7A9D8C]/50 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-4 focus:ring-[#7A9D8C]/10 transition-all duration-300 cursor-pointer">
              <option value="ios" class="text-gray-900">iOS Style</option>
              <option value="android" class="text-gray-900">Android Style</option>
            </select>
            <div class="absolute right-4 top-4 pointer-events-none text-white/30 text-[10px]">▼</div>
          </div>
        </div>
        <div>
          <label class="block text-[10px] font-medium text-white/40 uppercase tracking-widest mb-2">Wallpaper</label>
          <label class="flex items-center justify-center w-full h-[46px] bg-white/5 hover:bg-white/10 border border-white/10 border-dashed rounded-xl cursor-pointer transition-all duration-300 group hover:border-[#7A9D8C]/50">
            <span class="text-xs text-white/40 group-hover:text-[#7A9D8C] transition-colors">Upload Image</span>
    ...
    <div class="space-y-5 pt-4 border-t border-white/5">
      <div class="flex items-center gap-2 mb-2">
         ...
         <h3 class="text-sm font-medium text-white/80 tracking-wide">生成模式 / Mode</h3>
      </div>
      ...
      <div class="p-5 bg-white/5 rounded-2xl border border-white/5 space-y-5">
        <div>
          <div class="flex justify-between items-end mb-3">
             <label class="text-[10px] font-medium text-white/40 uppercase tracking-widest">Quantity</label>
             <span class="text-xs font-bold text-[#7A9D8C]">{{ genCount }} <span class="text-[10px] font-normal text-white/30">items</span></span>
          </div>
        ...
        >
          <span v-if="currentMode === 'chat'">Generate Dialogue</span>
          <span v-else>Generate Join Logs</span>
        ...
        <p class="text-center text-[10px] text-white/30 tracking-wide">
          Automatically generate realistic content based on selected mode
        </p>
      </div>
    </div>
  </div>
